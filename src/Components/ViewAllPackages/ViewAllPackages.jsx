...
const ViewAllPackages = () => {
    ...
    useEffect(() => {
        const fetchPackageData = async () => {
            await axios
                .get(`http://localhost:5000/viewAllPackages`)
                .then((res) => {
        ...
        { FilteredPackages.length !== 0 ? FilteredPackages.map((item , index) => (
            <div id='cards' key={index} className='flex justify-center items-center border-2 border-solid border-white rounded-lg p-6 mb-12 mx-8'>
                <div className='flex justify-start items-center w-4/6'>
                    ...
                    <div style={{backgroundImage:`url(${item.pics[ImgIdx]})`}} className='h-40 w-1/4 bg-cover text-white hover:scale-95 bg-cover bg-center text-white'></div>
                    <div className='pl-6'>
                        <h1 className='text-2xl text-white font-sans font-bold underline pb-4 hover:cursor-pointer hover:text-indigo-400 transition duration-300 ease-in-out active:scale-90'>{item.packageTitle.split('#')[0]}</h1>
                ...
                <div className='w-2/6 flex intems-center justify-end'>
                    <div>
                        ...
                        <div>
                            <button className='text-white text-xl font-sans font-bold bg-indigo-800 rounded-md p-2 hover:shadow-sm hover:shadow-indigo-300 hover:bg-indigo-600 transition duration-300 ease-in-out active:scale-75'>View Details <FontAwesomeIcon icon={faArrowRight} size='sm' fixedWidth /></button>
                        </div>
                    </div>
